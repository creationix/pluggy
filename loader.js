window.addEventListener("load", function () {
  ...
  if (window.hasgens) return start();

  var left = 2;
  console.log("No native generator support detected, loading regenerator compiler and runtime");
  loadScript("tedit-regenerator/regenerator-bundle.js", function (err, fn) {
    var exports = {};
  ...
  loadScript("tedit-regenerator/runtime.js", function (err, fn) {
    fn();
    if (!--left) return start();
  }, true);

  function start() {
    requireBase("").async("./main.js", function (err) {
      if (err) throw err;
    });
  }
  ...
  function loadScript(url, callback, raw) {
    if (!callback) return loadScript.bind(null, url);
    if (defs[url]) return callback(null, defs[url]);
    // TODO: add concurrency guard
    readUrl(url, function (err, js) {
      ...
      function inject(err) {
        ...
        if (regenerator) js = regenerator(js);
